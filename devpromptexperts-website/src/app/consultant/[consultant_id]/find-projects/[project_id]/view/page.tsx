...
export default function ViewProjectPage() {
    ...
    const loadData = async (id: string) => {
        try {
            ...
            // Check for existing response to get responseId for communications
            ...
            <div className="max-w-7xl mx-auto space-y-6">
                    ...
                    <div className="flex items-center space-x-4">
                        ...
                        <div>
                            ...
                            <div className="text-sm text-gray-500 dark:text-gray-400">
                                {activeTab === 'overview' ? 'Read-only View' : 'Messages'}
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="border-b border-gray-200 dark:border-gray-700 mb-6 font-primary">
                    <nav className="-mb-px flex space-x-8">
                        {['overview', 'communication'].map((tab) => (
                            <button
                                key={tab}
                                onClick={() => setActiveTab(tab as 'overview' | 'communication')}
                                className={`
                                    ...
                                    ${tab === 'communication' && !responseId ? 'opacity-50 cursor-not-allowed' : ''}
                                `}
                                disabled={tab === 'communication' && !responseId}
                                title={tab === 'communication' && !responseId ? "Submit a response first to enable communication" : ""}
                            >
                                {tab}
                            </button>
                        ))}
                ...
                {activeTab === 'overview' ? (
                    ...
                    responseId && (
                        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-100 dark:border-gray-700 overflow-hidden min-h-[600px] flex flex-col">
                            <ProposalCommunications responseId={responseId} />
                        </div>
                    )
                )}
            </div>
