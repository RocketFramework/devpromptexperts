// 🌍 CountryToFlag.ts
// Converts any recognized country name or ISO code to its flag emoji
// Includes all 195 UN-recognized countries (as of 2025)
...
// Utility: convert ISO code (e.g. "US") → 🇺🇸
...
// Main function
export const CountryToFlag = (country: string | null): string => {
  if (!country) return "🏴";
  const name = country.toLowerCase().trim();

  // If user passes an ISO code directly
  if (name.length === 2 && /^[a-z]{2}$/.test(name)) {
    return isoToFlag(name);
  }

  const isoCode = countries[name];
  if (isoCode) {
    return isoToFlag(isoCode);
  }

  return "🏴"; // default for unrecognized input
};
