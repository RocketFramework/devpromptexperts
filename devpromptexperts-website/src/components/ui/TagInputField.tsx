...
export const TagInputField: FC<TagInputFieldProps> = ({
  ...
  const handleInputChange = (value: string) => {
    if (disabled) return;
    tagInput.handleInputChange(value);
  };

  return (
    <div className="w-full">
      <label className={`block text-sm font-medium mb-3 ${darkMode ? 'text-gray-300' : 'text-gray-700'} ${disabled ? 'opacity-60' : ''}`}>
        {label} {required && '*'}
      </label>
      ...
      <div className="relative">
        <div 
          className={`flex flex-wrap gap-2 p-3 border rounded-lg min-h-12 transition-colors ${
            disabled 
            ...
            tagInput.showSuggestions && !disabled ? 'ring-2 ring-blue-500 border-blue-500' : ''
          }`}
        ...
        >
          ...
          <input
            ...
            onFocus={() => !disabled && tagInput.setShowSuggestions(true)}
            onBlur={() => !disabled && setTimeout(() => tagInput.setShowSuggestions(false), 200)}
        ...
        {!disabled && tagInput.showSuggestions && tagInput.suggestions.length > 0 && (
          <div 
      ...
      {/* Helper Text */}
      {helperText && (
        <p className={`mt-2 text-xs ${darkMode ? 'text-gray-400' : 'text-gray-500'} ${disabled ? 'opacity-60' : ''}`}>
          {helperText}
        </p>
      )}
    </div>
