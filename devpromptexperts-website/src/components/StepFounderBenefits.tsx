...
export default function StepFounderBenefits({ data, onUpdate, onNext, onBack }: StepFounderBenefitsProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8">
      <div>
        <h2 className="text-2xl font-bold text-gray-900 mb-2">Founder 100 Benefits</h2>
        <p className="text-gray-600">Unlock exclusive opportunities as a founding member</p>
      </div>
      ...
      <div className="bg-blue-50 rounded-2xl p-6 border border-blue-200">
        <div className="flex items-start space-x-4">
          ...
          <div>
            <h3 className="text-lg font-semibold text-gray-900 mb-2">Founder 100 Exclusive Benefits</h3>
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between p-4 border border-gray-200 rounded-lg">
          <div>
            ...
            <p className="text-sm text-gray-600">Express interest in equity consideration</p>
          </div>
          <label className="relative inline-flex items-center cursor-pointer">
            <input
              type="checkbox"
              checked={data.interestedInEquity}
              onChange={(e) => onUpdate({ interestedInEquity: e.target.checked })}
        ...
        <div className="flex items-center justify-between p-4 border border-gray-200 rounded-lg">
          <div>
            ...
            <p className="text-sm text-gray-600">Help shape the future of the platform</p>
          </div>
          <label className="relative inline-flex items-center cursor-pointer">
            <input
              type="checkbox"
              checked={data.wantAdvisoryRole}
              onChange={(e) => onUpdate({ wantAdvisoryRole: e.target.checked })}
              className="sr-only peer"
            />
            <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-600"></div>
          </label>
        ...
        <div>
          ...
          <textarea
            ...
            onChange={(e) => onUpdate({ referralContacts: e.target.value })}
            rows={3}
