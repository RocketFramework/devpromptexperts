...
export const consultantDTOs = Array.from({ length: 100 }, (_, i) => {
  ...
  return {
    ...
    bioSummary: `Experienced ${base.title} with expertise in ${(
      base.expertise ?? []
    ).join(", ")}.`,
    ...
    rating:
      Math.round((base.rating ?? 0 + (Math.random() - 0.5) * 0.4) * 10) / 10,
    projects_completed:
      base.projectsCompleted ?? 0 + Math.floor(Math.random() * 50),
    featured: i < 10 ? true : false, // First 10 are featured
  } as ConsultantDTO;
});

export const featuredConsultants = consultantDTOs.filter((c) => c.featured);
export const allExpertise = Array.from(
  new Set(consultantDTOs.flatMap((c) => c.expertise))
);
